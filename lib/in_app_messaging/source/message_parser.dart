...
/// glyph and has no scrim to tap — both by design, because a banner that size
/// has no room for either — so its only affordances are a swipe
...
/// told them about.
///
/// Swipe-to-dismiss and auto-dismiss are two independent fields here, so "no
/// exit at all" is representable — and campaigns do reach that state. This
/// default makes it unrepresentable again, at the parser, rather than asking
/// every campaign author to remember.
///
/// Eight seconds: long enough to read three lines of copy, short enough that a
/// banner nobody interacts with clears itself well inside a session.
...
InAppMessageCampaign? _parseCampaign(Map<String, dynamic> json) {
  ...
  final triggerJson = json['trigger'] is Map<String, dynamic>
...
GameballMessageTrigger? _parseTrigger(Object? json, String label) {
  ...
  switch (type) {
    ...
    case 'custom_event':
      ...
      final filters =
          _parseFilters(json['metadataFilters'] ?? json['filters'], label);
      if (filters == null) {
...
GameballFilterOperator? _parseOperator(Object? value) {
  return switch (_asString(value)?.toLowerCase().replaceAll('_', '')) {
    // The sync carries the dashboard's enum names verbatim — Is, Greater, Less,
    // Between, Equals, NotEquals, Contains — so those are accepted alongside the
    // long spellings. An unrecognised operator must not silently drop the
    // filter: that would widen the campaign to every occurrence of its event.
    'is' ||
    'equals' ||
    'equal' ||
    'eq' ||
    ...
    'less' || 'lessthan' || 'lt' || '<' => GameballFilterOperator.lessThan,
    'lessorequal' ||
    'lessthanorequal' ||
    'lessthanorequals' ||
    'lte' ||
    ...
    'between' => GameballFilterOperator.between,
...
GameballInAppMessage? _parseMessage(
  ...
  // A slideup has no room for buttons beside three lines of text. Dropped
  // loudly rather than silently, since a campaign that configured them expected
  // them to appear.
  var buttons = _parseButtons(content['buttons'], locale['buttons'], label);
  if (type == GameballMessageType.slideup && buttons.isNotEmpty) {
    iamLog(
        'campaign $label: ${buttons.length} button(s) ignored — a slideup has '
...
///
/// The backend names this explicitly — `layout` on `content`. Until it did,
/// this function inferred the layout from which fields arrived, which could not
/// tell a deliberately image-only campaign from one whose personalised copy
/// resolved to empty. That inference is gone, along with the alternate keys and
/// spellings that hedged against a contract which had not been written.
///
...
///
/// Defaults to the bottom rather than the top: a top banner covers the status
/// bar and whatever app-bar control sits under it.
...
GameballClickAction _parseAction(Object? json, String messageId) {
  ...
  switch (type) {
    ...
    case 'open_url':
      ...
      return GameballOpenUrlAction(url,
          external: _asBool(json['external']) ?? false);
    case 'navigate':
    ...
    case 'request_push_permission':
      return const GameballRequestPushPermissionAction();
    default:
      // A working close beats a dead button.
      iamLog(
          'message "$messageId": unsupported action type "$type", using dismiss');
      return const GameballDismissAction();
  }
}
...
Map<String, String> _parseExtras(Object? json) {
  ...
  json.forEach((key, value) {
    if (key is String && value != null) {
      // Coerced rather than dropped: discarding non-string extras would lose
      // campaign data with no diagnostic.
...
///
/// The integer form exists because some payloads encode colours as packed ARGB
/// integers rather than hex strings.
