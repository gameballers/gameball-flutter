...
abstract final class MessageMetrics {
  ...
  /// nothing about a modal argues for a smaller control. Kept separate from the
  /// hit target, which stays 48 everywhere and is the accessibility floor on
  /// both platforms.
...
abstract final class ModalMetrics {
  ...
  /// nothing a campaign realistically ships letterboxes, a 3:5 poster included.
  /// Without a cap at all, a portrait poster would grow the card until the copy
  /// was crushed into a 23-point sliver.
  static const double minImageRatio = 0.55;

  /// Height always kept for the copy and buttons, whatever the artwork wants.
  ///
  /// Roughly one line of copy plus a button block. Without it, a required
  /// aspect constraint has to win on a small screen, which means a broken
  /// constraint rather than a considered outcome. This bars the image slightly
  /// on a genuinely cramped device instead — the one place bars remain, and the
  /// better failure.
...
abstract final class FullscreenMetrics {
  ...
  /// composition.
  ///
  /// A fixed share, not "whatever the copy leaves", for the same reason that
  /// drove the modal: an image sized by subtraction lands on whatever ratio is
  /// left over and letterboxes when that does not match the artwork. Half of the
  /// *safe* area rather than half the screen, because the stack stays inside it
  /// rather than running under the status bar.
...
/// which is exactly the case a campaign should name a colour for.
