...
/// The modal layout, covering both modal variants.
///
...
class GameballInAppMessageModal extends StatelessWidget {
              ...
              child: Stack(
                children: [
                  if (_imageOnly)
                    ...
                    // outside it, which keeps a call to action on screen
                    // under a tall poster: the artwork takes the height it
  ...
  ///
  /// The same composition as the fullscreen image-only variant: the artwork
  /// carries the message and the call to action sits on top of it. What this
  /// replaces put the image in a
  ...
  ///
  /// Deliberately not `BoxFit.cover` with a fixed height: cropping to a fixed
  /// band discards the design a marketer uploaded — for an image-only message it
  /// would reduce a portrait poster to an unreadable horizontal slice.
  Widget _image(
