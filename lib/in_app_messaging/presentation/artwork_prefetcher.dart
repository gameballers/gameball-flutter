...
class ImageCacheArtworkPrefetcher implements ArtworkPrefetcher {
  ...
  Future<bool> _load(String url) {
    // Cleartext artwork is refused, not attempted. iOS App Transport Security
    // and Android's cleartext policy (API 28+) both block it by default, so on
    // the native SDKs such a campaign never shows. Dart's own HTTP stack answers
    // to neither policy, so without this check the same campaign would display
    // here and nowhere else. One rule on every platform: https only — and the
    // campaign is passed over exactly as it is when artwork fails to load.
    if (url.startsWith('http://')) {
      iamLog('artwork "$url" is served over http and is refused — in-app '
          'artwork must be https');
