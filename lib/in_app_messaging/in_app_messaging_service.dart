...
///
/// Synchronous, so it cannot delay the frame the message is drawn in. If it
...
/// Asks the OS for notification permission; true when it was granted.
typedef PushPermissionRequester = Future<bool> Function();
...
///
/// Return false to take over the action entirely — useful when the host needs
/// to route a click through its own analytics or navigation stack.
...
/// can never block a warm session-start message. A shorter timeout opens a gap
/// where a session-start campaign is selected and then silently suppressed.
///
...
///
/// Lapsing is not free: sync substitutes, so the text already holds on
...
/// Skipping a message whose artwork did not load is right; caching the *reason*
/// for a whole session is not.
///
...
class InAppMessagingService {
  InAppMessagingService({
    ...
    PushPermissionRequester? pushPermission,
    ...
    this.artworkRetryInterval = defaultArtworkRetryInterval,
        ...
        _requestPushPermission = pushPermission ?? _defaultPushPermission;
  ...
  /// Injected so tests can fire the floor retry without waiting real seconds.
  final Timer Function(Duration, void Function()) _timer;
  final UrlLauncher _launcher;
  final PushPermissionRequester _requestPushPermission;
  ...
  /// The daily local-time window during which nothing displays, as the last
  /// sync reported it. Null when the account has none.
  GameballQuietHours? _quietHours;

  /// The one message waiting for a display opportunity, if any.
  ///
  /// A single slot: a newer deferral displaces an older one. For one message
  /// at a time a slot is honest and enough.
  InAppMessageCampaign? _pending;

  /// Fires [_retryPending] when the display floor that parked [_pending] lapses.
  Timer? _floorRetry;
  ...
  /// Asks through Firebase Messaging, which this SDK already ships for push.
  /// The host may inject its own requester (a permission plugin, a pre-prompt)
  /// through the constructor; this is the behaviour when it does not.
  static Future<bool> _defaultPushPermission() async {
  ...
  Future<void> _syncAndEvaluateSessionStart(
      {bool loadPersisted = false}) async {
  ...
  Future<bool> _isArtworkReady(InAppMessageCampaign campaign) async {
    try {
      return await _prefetcher
          .prefetch(campaign.message)
  ...
  void _maybeRefreshArtwork(List<InAppMessageCampaign> notReady) {
    ...
    if (last != null && _clock().difference(last) < artworkRetryInterval)
      return;
  ...
  void _evaluate(GameballTriggerOccurrence occurrence) {
    ...
    switch (_decide(campaign.message)) {
      ...
      case GameballDisplayDecision.show:
        // Snapshotted before the winner displays: both campaigns fired on the
        // same occurrence and are judged against the same state. Otherwise the
        // winner's own impression starts the floor that rules its runner-up out.
        final capBefore = _cap.snapshot();
        _tryPresent(campaign);
        _deferRunnerUp(occurrence, displayable, campaign, capBefore);
    }
  }

  /// Keeps the next-best campaign for this occurrence waiting behind the one on
  /// screen, so dismissing a High message reveals the Normal one that fired on
  /// the same trigger instead of losing it. Only when the winner actually took
  /// the screen: a deferred winner already holds the pending slot and must not
  /// be displaced by its own runner-up. Whether the runner-up may still display
  /// when its turn comes is decided then, by the repeat, cap and cooldown checks
  /// every retry goes through.
  void _deferRunnerUp(
    GameballTriggerOccurrence occurrence,
    List<InAppMessageCampaign> displayable,
    InAppMessageCampaign winner,
    CapState capBefore,
  ) {
    if (!_presenter.isShowing || _pending != null) return;

    final runnerUp = selectCampaign(
      occurrence: occurrence,
      campaigns: displayable
          .where((candidate) => candidate.campaignId != winner.campaignId)
    ...
    _defer(runnerUp, 'campaign "${winner.campaignId}" took this trigger first');
  }
  ...
  void _present(InAppMessageCampaign campaign, GameballInAppMessage message) {
    ...
    final presented = _presenter.present(
      ...
      onDismissed: () {
        // Without this event "shown and ignored" would be invisible in
        // reporting: closing the glyph, tapping outside and backgrounding the
        // app would all log nothing. It costs one event.
        if (shown && !engaged) {
  ...
  void _defer(InAppMessageCampaign campaign, String reason) {
    ...
    if (displaced != null && displaced.campaignId != campaign.campaignId) {
      iamLog(
          'pending campaign "${displaced.campaignId}" displaced by "${campaign.campaignId}"');
    }
    _pending = campaign;
  ...
  void _retryPending() {
    ...
    if (isWithinFloor(capState: capState, now: _clock(), cooldown: _cooldown)) {
      _pending = campaign;
      _scheduleFloorRetry(capState.lastDisplayAt);
      return;
    }

    // The window may have opened while the message waited. Dropped rather than
    // parked, for the same reason the evaluator suppresses instead of deferring:
    // a quiet window is hours long, and a message held that long would arrive
    // as a non sequitur to whatever it was answering.
    final quiet = _quietHours;
    if (quiet != null && quiet.contains(_clock())) {
      iamLog('pending campaign "${campaign.campaignId}" dropped: it is now '
  ...
  /// Retries [_pending] the moment the display floor lapses.
  ///
  /// Without this, a campaign parked inside the floor waited for the *next*
  /// dismissal or widget close. A runner-up behind a winner dismissed within
  /// the cooldown could sit in the slot for the rest of the session.
  void _scheduleFloorRetry(DateTime? lastDisplayAt) {
    if (lastDisplayAt == null) return;
    final remaining = _cooldown - _clock().difference(lastDisplayAt);
    _floorRetry?.cancel();
    _floorRetry = _timer(remaining.isNegative ? Duration.zero : remaining, () {
      _floorRetry = null;
      _retryPending();
    });
  }
  ...
  Future<void> _runAction(GameballClickAction action) async {
    switch (action) {
      ...
      case GameballRequestPushPermissionAction():
        try {
          final granted = await _requestPushPermission();
          iamLog('push permission ${granted ? 'granted' : 'not granted'}');
        } catch (error) {
          // A host without Firebase initialised, or a platform that has no
          // prompt, must not turn the tap into a crash; the message is already
          // dismissed.
          iamLog('could not request push permission ($error)');
        }
    }
  }
}
