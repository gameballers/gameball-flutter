...
/// a real layout: a silent fallback would render the wrong message type instead
/// of reporting that the client cannot draw it. Prefer an explicit unknown over
/// a default that lies.
...
/// because nothing in the payload names it. Held on the model rather than worked
/// out in each widget, so that when the backend sends the layout explicitly one
/// line in the parser replaces the guess and nothing else changes.
enum GameballMessageLayout {
  /// Image above the copy, buttons beneath it.
  textWithImage,
...
/// Asks the OS for notification permission, then closes the message.
///
/// The dashboard offers this as a button action so a campaign can be the moment
/// that asks. Without a type of its own it would reach the device as an unknown
/// action and silently behave as a close button.
final class GameballRequestPushPermissionAction extends GameballClickAction {
  const GameballRequestPushPermissionAction();
}

/// Navigates to a named route in the host app, then closes the message.
///
/// A route, not a URI handed to the OS: the alternative would leave the app,
/// come back in through a platform link, and need native code to catch it and
/// forward it to Dart over a channel the integrator writes themselves. Because
/// this SDK renders in Flutter and already
...
class GameballMessageButton {
  ...
  /// styling and in every locale's labels, so a button keeps its identity when
  /// the copy around it is translated.
  final String id;
  final String text;
...
///
/// Covers both modal layouts. "Text (with Optional Image)" sets [body]; "Image
/// Only" sets [imageUrl] and leaves [header] and [body] null. At
...
class GameballInAppMessage {
  ...
  /// types.
  ///
  /// Defaults to the bottom, which is the safer choice: a banner at the top
  /// competes with the status bar and whatever app-bar action sits underneath
  /// it.
