...
///
/// **The times are the device's local wall clock.** Decided with product on
/// 2026-09-05, superseding the 2026-08-24 reading that they were UTC. The wire
/// carries the bare `HH:mm` exactly as the marketer typed it in the dashboard —
/// verified on alpha: a window saved as 03:00–04:00 arrives as 03:00–04:00 — and
/// the dashboard promises "22:00 means 22:00 wherever the customer is". The
/// difference is not cosmetic: at UTC+3 the two readings disagree for six hours
/// of every day.
class GameballQuietHours {
  ...
  /// Minutes from local midnight at which it closes. Exclusive.
  final int endMinute;

  /// Whether [instant] falls inside the window on the **device's local clock**.
  ///
  /// [instant] is converted rather than assumed, so a UTC `DateTime` and the
  /// local one for the same moment give the same verdict — the answer depends on
  /// the instant and the device's zone, never on how the caller expressed it.
  /// A customer in Cairo and one in Lisbon each get their own 22:00.
  ///
  ...
  bool contains(DateTime instant) {
    ...
    // A window that wraps past midnight — 22:00 to 08:00 — is two ranges,
