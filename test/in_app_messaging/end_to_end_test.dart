...
void main() {
  ...
  testWidgets('styling from the payload reaches the rendered surface',
    ...
    expect(surface.color, const Color(0xFFFFFFFF),
        reason: 'the fixture sets backgroundColor #FFFFFF, and the campaign\'s '
            'own styling must survive the whole parse-to-paint path');
  });
  ...
  testWidgets('tapping a button dismisses the message', (tester) async {
    ...
    // The fixture's second session-start campaign is deferred behind the
    // first and armed to retry when the floor lapses; stop cancels that timer
...
''';
    ...
    testWidgets('one purchase produces one trigger occurrence', (tester) async {
      ...
      expect(seen, ['9001/1'],
          reason:
              'logPurchase sends the reserved event, which already notifies '
