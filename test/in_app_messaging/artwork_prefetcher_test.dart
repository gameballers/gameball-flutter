...
void main() {
  ...
  test('refuses cleartext artwork without resolving it (G4)', () async {
    var resolved = 0;
    final prefetcher = ImageCacheArtworkPrefetcher(
      provider: (url) {
        resolved++;
    ...
    final ready =
        await prefetcher.prefetch(message(imageUrl: 'http://cdn/hero.png'));

    expect(ready, isFalse, reason: 'http artwork never counts as ready');
    expect(resolved, 0, reason: 'refused before any fetch is attempted');
  });
