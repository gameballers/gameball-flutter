...
void main() {
  group('the payload root', () {
    ...
    test('a bots-style wrapper is no longer unwrapped', () {
      ...
      final result =
          parseSyncResponse('{"success":true,"response":{"cooldownSeconds":45,'
              '"messages":[${minimal()}]}}');
  ...
  group('closeBehaviour', () {
    ...
    test('swipe on a fullscreen campaign is promoted to both', () {
      ...
      final m = one(minimal(content: '{"closeBehaviour":"swipe"}')
              .replaceFirst('"messageType": 2', '"messageType": 3'))!
          .message;
  ...
  group('triggers', () {
    ...
    test('drops an event trigger with only a numeric id', () {
      expect(
        ...
        isNull,
        reason:
            'the id is meaningless on the device — there is nothing to match '
  ...
  group('metadata filters', () {
    ...
    test(
        'reads it from metadataName too, since the spelling was agreed verbally',
        () {
      final t = eventTrigger('[{"metadataId":1,"name":"price","operator":"is",'
          '"value":"x"}]')!;
    ...
    test('drops a filter with an unusable operator, widening the campaign', () {
      final t =
          eventTrigger('[{"name":"a","operator":"sounds_like","value":1}]')!;

      expect(t.filters, isEmpty,
          reason:
              'a typo in one operator should not hide the campaign entirely');
    });

    test('drops a filter with no value', () {
      expect(eventTrigger('[{"name":"a","operator":"is"}]')!.filters, isEmpty);
    });
  ...
  group('expiry', () {
    ...
    test('hasExpiredAt is inclusive of the expiry instant', () {
      ...
      expect(campaign.hasExpiredAt(DateTime.utc(2026, 9, 30, 21, 59, 58)),
          isFalse);
      expect(
          campaign.hasExpiredAt(DateTime.utc(2026, 9, 30, 21, 59, 59)), isTrue);
    });
  });
  ...
  group('leniency', () {
    test('matches enum-ish values case-insensitively', () {
      final m = one(minimal(
        trigger: '{"type":"SESSION_START"}',
        content:
            '{"closeBehaviour":"BOTH","textAlignment":{"header":"CENTER"}}',
      ))!
          .message;
    ...
    test('treats a non-positive autoDismissSeconds as no auto-dismiss', () {
      expect(
          one(minimal(content: '{"autoDismissSeconds":0}'))!
              .message
              .autoDismissAfter,
          isNull);
      expect(
          one(minimal(content: '{"autoDismissSeconds":-3}'))!
              .message
              .autoDismissAfter,
          isNull);
    });
  ...
  group('modal layouts', () {
    ...
    test('a message-level dismiss action is honoured', () {
      expect(
          one(minimal(content: '{"action":{"type":"dismiss"}}'))!
              .message
              .clickAction,
          isA<GameballDismissAction>());
    });
    ...
    test('an unusable message action leaves the surface untappable', () {
      expect(
        ...
        isNull,
        reason:
            'silently turning the whole message into a close button would be '
    ...
    test('the message action is independent of button actions', () {
      ...
      expect(
          (m.buttons.single.action as GameballOpenUrlAction).url, 'https://b');
    });
  });
  ...
  group('never throws', () {
    ...
    test('returns empty when messages is not a list', () {
      expect(parseSyncResponse('{"response":{"messages":"x"}}').campaigns,
          isEmpty);
    });

    test('skips non-object entries in messages', () {
      expect(
          parseSyncResponse('{"response":{"messages":[1,"x",null]}}').campaigns,
          isEmpty);
    });
  });
  ...
  group('slideup', () {
    String slideup(
            {String content = '{}', String locale = '{"message":"hi"}'}) =>
        '{"campaignId":1,"messageType":1,"trigger":{"type":"session_start"},'
        '"content":$content,"locale":$locale}';

    test('reads slideFrom', () {
      expect(
          one(slideup(content: '{"slideFrom":"top"}'))!.message.slidePosition,
          GameballSlidePosition.top);
      expect(
          one(slideup(content: '{"slideFrom":"bottom"}'))!
              .message
              .slidePosition,
    ...
    test('defaults to the bottom when slideFrom is absent or unknown', () {
      expect(
          one(slideup())!.message.slidePosition, GameballSlidePosition.bottom);
      expect(
    ...
    test('drops buttons, loudly', () {
      ...
      expect(message.buttons, isEmpty,
          reason: 'there is no room for buttons beside three lines of text — '
              'the whole surface is the tap target instead');
    });
    ...
    test('needs text: an icon alone is not a message', () {
      expect(
        ...
        isNull,
        reason:
            'a 40-point square with no words says nothing. A modal can carry '
    ...
    test('an image-only slideup is dropped even with an imageUrl', () {
      expect(
        one(slideup(
            content: '{"imageUrl":"https://cdn/big.png"}', locale: '{}')),
        isNull,
      );
    });
    ...
    test('accepts header as a fallback when message is absent', () {
      expect(
          one(slideup(locale: '{"header":"From the header"}'))!.message.header,
          'From the header');
    });
  ...
  group('layout, which the backend now names explicitly', () {
    ...
    test('a modal honours text_with_image', () {
      expect(
        one(full(content: '{"layout":"text_with_image",$image}'))!
            .message
            .layout,
        GameballMessageLayout.textWithImage,
      );
    ...
    test('alternate wire spellings are no longer read', () {
      expect(
        one(full(messageType: 2, content: '{"layout":"graphic",$image}'))!
            .message
            .layout,
        GameballMessageLayout.textWithImage,
        reason: 'this SDK follows the Gameball contract only',
      );
    });
  });
  ...
  group('artwork, which fullscreen delivers as media', () {
    ...
    test('a fullscreen prefers media over imageUrl', () {
      final message = one(withContent('{"imageUrl":"https://x/i.png",'
    ...
    test('a blank url is treated as absent', () {
      final message =
          one(withContent('{"imageUrl":"   ","media":null}', messageType: 2))!
              .message;
      expect(message.imageUrl, isNull,
    ...
    test('a blank icon url is treated as absent', () {
      final message = one('{"campaignId":1,"messageType":1,'
              '"trigger":{"type":"session_start"},'
              '"content":{"iconUrl":"  "},"locale":{"message":"hi"}}')!
          .message;
  ...
  group('orientation', () {
    ...
    test('reads portrait and landscape', () {
      expect(
          one(full(content: '{"orientation":"portrait"}'))!.message.orientation,
          GameballMessageOrientation.portrait);
      expect(
          one(full(content: '{"orientation":"landscape"}'))!
              .message
              .orientation,
    ...
    test('defaults to any when absent or unknown', () {
      ...
      expect(
          one(full(content: '{"orientation":"sideways"}'))!.message.orientation,
...
''');
    ...
    test('a response without them has none', () {
      final result =
          parseSyncResponse('{"cooldownSeconds": 60, "messages": []}');
...
''';
    ...
    test('one with no close behaviour and no duration gets the default', () {
      ...
      // sits on the host's app bar until then. Real campaigns do reach that
      // state, which is why the parser supplies a default.
      final message =
          parseSyncResponse(slideupPayload()).campaigns.single.message;
...
''').campaigns.single.message;
  ...
  group('dashboard enum operator names (G12)', () {
    GameballCustomEventTrigger? eventTrigger(String filters) {
      final campaign = one(minimal(
        trigger: '{"type":"event","name":"e","metadataFilters":$filters}',
      ));
    ...
    test('Greater, Less and Between are kept, not dropped', () {
      expect(
        eventTrigger('[{"name":"amount","operator":"Greater","value":"100"}]')!
            .filters
            .single
            .operator,
      ...
      expect(
        eventTrigger('[{"name":"amount","operator":"Less","value":"10"}]')!
            .filters
            .single
            .operator,
        GameballFilterOperator.lessThan,
      );
      final between = eventTrigger(
              '[{"name":"amount","operator":"Between","value":"20,30"}]')!
          .filters
          .single;
      expect(between.operator, GameballFilterOperator.between);
      expect(between.value, '20,30');
    });

    test('a Between filter narrows the trigger instead of widening it', () {
      final t = eventTrigger(
          '[{"name":"amount","operator":"Between","value":"20,30"}]')!;
      expect(
        allFiltersMatch(t.filters, <String, Object>{'amount': 25}),
        isTrue,
      );
      expect(
        allFiltersMatch(t.filters, <String, Object>{'amount': 5}),
        isFalse,
      );
    });
  });

  group('cleartext artwork (G4)', () {
    test('an http:// image is kept on the model; the prefetcher refuses it',
        () {
      final m = one(minimal(
        content: '{"imageUrl":"http://example.com/qa/IMG-1.png"}',
      ))!
          .message;
      // The parser does not judge the scheme — ImageCacheArtworkPrefetcher
      // does, so the campaign is passed over exactly like unloadable artwork.
      expect(m.imageUrl, 'http://example.com/qa/IMG-1.png');
    });
  });

  group('request_push_permission (G2)', () {
    test('parses to its own action instead of a dismiss', () {
      final m = one(minimal(
        content:
            '{"buttons":[{"id":"primary","action":{"type":"request_push_permission"}}]}',
        locale: '{"message":"m","buttons":[{"id":"primary","text":"Allow"}]}',
      ))!
          .message;
      expect(
          m.buttons.single.action, isA<GameballRequestPushPermissionAction>());
    });
  });
}
