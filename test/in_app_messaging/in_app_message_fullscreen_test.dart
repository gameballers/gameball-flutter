...
void main() {
  ...
  group('the stacked variant', () {
    ...
    testWidgets('fills the width rather than letterboxing', (tester) async {
      // `contain` bars a portrait poster by 35 either side on a 390-wide
      // screen. A stacked fullscreen reads as a poster with copy under it, not
      // as a framed picture, so it crops instead.
      await pump(tester, fullscreen());
    ...
    testWidgets('takes exactly half the available height', (tester) async {
      // A fixed half of the content height rather than whatever slack the
      // copy leaves. Half of the *safe* area rather than half the screen,
      // because the stack stays inside it rather than running under the
      // status bar.
