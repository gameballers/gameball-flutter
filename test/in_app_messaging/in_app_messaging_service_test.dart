...
class FakePresenter implements GameballMessagePresenter {
  ...
  void tapButton(GameballMessageButton button) =>
      _onButtonPressed?.call(button);
...
class RecordingAnalytics implements MessageAnalytics {
  ...
  List<String> get bodyClicks => events
      .where(
          (e) => e.type == GameballMessageEventType.click && e.buttonId == null)
      .map((e) => labelOf(e.campaignId))
      .toList();
...
({
  ...
  List<(Duration, void Function())> scheduled,
}) build({
  List<InAppMessageCampaign>? campaigns,
  InMemoryCampaignCache? cache,
  ...
  final service = InAppMessagingService(
    ...
    timer: (duration, callback) {
      scheduled.add((duration, callback));
      return _ManualTimer();
    },
...
/// A timer the test fires by hand, through the `scheduled` list.
...
void main() {
  ...
  group('deferral and retry', () {
    ...
    test('the runner-up on the same trigger waits behind the winner (D-31)',
        () async {
      final h = build(campaigns: [
        campaign('high',
            trigger: const GameballCustomEventTrigger('add_to_cart'),
      ...
      h.source.cooldown = Duration.zero;
      await h.service.start(customerId: 'c1');

      h.service.onCustomEvent('add_to_cart');

      expect(h.presenter.shownMessageIds, ['msg_high']);
      expect(h.service.pendingCampaign?.campaignId, idFor('low'),
          reason: 'the Normal campaign that fired on the same event is kept, '
              'not lost');

      h.presenter.dismiss();

      expect(h.presenter.shownMessageIds, ['msg_high', 'msg_low']);
      expect(h.service.pendingCampaign, isNull);
    });

    test('a runner-up parked by the cooldown shows once it lapses (D-31)',
        () async {
      final h = build(campaigns: [
        campaign('high',
            trigger: const GameballCustomEventTrigger('add_to_cart'),
      ...
      h.source.cooldown = const Duration(seconds: 10);
      await h.service.start(customerId: 'c1');
      h.service.onCustomEvent('add_to_cart'); // high shows at t0

      // Dismissed 3s in: the floor still has 7s to run, so low is parked.
      h.setNow(t0.add(const Duration(seconds: 3)));
      h.presenter.dismiss();

      expect(h.presenter.shownMessageIds, ['msg_high']);
      expect(h.service.pendingCampaign?.campaignId, idFor('low'));
      expect(h.scheduled, hasLength(1));
      expect(h.scheduled.single.$1, const Duration(seconds: 7),
          reason: 'the retry is timed to the end of the floor, not polled');

      h.setNow(t0.add(const Duration(seconds: 10)));
      h.scheduled.single.$2();

      expect(h.presenter.shownMessageIds, ['msg_high', 'msg_low']);
      expect(h.service.pendingCampaign, isNull);
    });

    test('a deferred winner is not displaced by its own runner-up', () async {
      final h = build(campaigns: [
        campaign('high',
            trigger: const GameballCustomEventTrigger('add_to_cart'),
            priority: 100),
      ...
      expect(h.presenter.shownMessageIds, isEmpty);
      expect(h.service.pendingCampaign?.campaignId, idFor('high'));
    });
    ...
    test('a pending message already shown is dropped rather than repeated',
        () async {
      final h = build(campaigns: [
        campaign('cart',
            trigger: const GameballCustomEventTrigger('add_to_cart')),
    ...
    test('the pending message displays when the current one is dismissed',
        () async {
      final h = build(campaigns: [
        campaign('first'),
        campaign('cart',
  ...
  group('beforeDisplay hook', () {
    ...
    test('a throwing hook falls back to show', () async {
      ...
      expect(h.presenter.shownMessageIds, ['msg_a'],
          reason:
              'the no-hook default is show, so falling back to it is least surprising');
    });

    test('the hook still receives messages it will not see displayed',
        () async {
      final seen = <String>[];
      final h = build();
      ...
      final h = build(campaigns: [
        campaign('a', buttons: const [
          GameballMessageButton(
              id: 'b5', text: 'Go', action: GameballDismissAction()),
        ]),
      ]);
      await h.service.start(customerId: 'c1');
  ...
  group('dismissal', () {
    ...
    test('a button tap suppresses the dismiss that follows it', () async {
      const button = GameballMessageButton(
          id: 'b1', text: 'Go', action: GameballDismissAction());
      final h = build(campaigns: [
        campaign('a', buttons: const [button])
      ]);
      await h.service.start(customerId: 'c1');
    ...
    test('every event carries the campaign token and the injected clock',
      ...
      expect(h.analytics.events.map((e) => e.dispatchId), ['tok_a', 'tok_a'],
          reason:
              'the token has to ride every event, not just the impression — '
              'a click the backend cannot attribute to a variant is no better '
              'than no click');
      expect(h.analytics.events.map((e) => e.occurredAt), [t0, t0],
          reason:
              'timestamps come from the service clock, not the transport, so '
  ...
  group('navigate action', () {
    ...
    test('the message is dismissed before the route is pushed', () async {
      ...
      expect(h.presenter.isShowing, isFalse,
          reason:
              'leaving the overlay up would briefly cover the pushed route');
    });
  ...
  group('purchase triggers', () {
    ...
    test('a price filter narrows a purchase trigger', () async {
      final h = build(campaigns: [
        campaign('big',
            trigger: const GameballCustomEventTrigger(gameballPurchaseEventName,
                filters: [
                  GameballPropertyFilter(
                    property: 'price',
  ...
  group('sessions', () {
    ...
    test(
        'a server cooldown longer than the session timeout suppresses, '
  ...
  group('quiet hours', () {
    // Local literals: the window is the marketer's HH:mm judged on the device's
    // own clock (decided 2026-09-05), so a UTC literal here would pass or fail
    ...
    test('a message that would display is suppressed inside the window',
      ...
      h.setNow(DateTime(2026, 8, 24, 23, 30));
    ...
    test('the same message displays outside the window', () async {
      final h = build(campaigns: [campaign('a')]);
      h.source.quietHours = night;
      h.setNow(DateTime(2026, 8, 24, 12, 0));
    ...
    test('an event inside the window is suppressed too', () async {
      ...
      h.source.quietHours = night;
      h.setNow(DateTime(2026, 8, 25, 3, 0));
      ...
      expect(h.presenter.shownMessageIds, isEmpty,
          reason:
              'the window applies to every trigger, not just session start');
    });
    ...
    test('suppression costs the occurrence, not the campaign', () async {
      ...
      h.setNow(DateTime(2026, 8, 25, 9, 0));
      h.service.onCustomEvent('promo');
      await pumpEventQueue();
    ...
    test('a message deferred before the window opened is dropped at retry',
        () async {
      final h = build(campaigns: [
        campaign('promo', trigger: const GameballCustomEventTrigger('promo')),
      ]);
      h.source.quietHours = night;
      h.setNow(DateTime(2026, 8, 24, 20, 0)); // daytime: window not yet open
      await h.service.start(customerId: 'c1');
      h.setWidgetOpen(true);

      h.service.onCustomEvent('promo');
      expect(h.service.pendingCampaign?.campaignId, idFor('promo'),
          reason: 'deferred because the Gameball widget is open');

      // The widget closes at 23:00, inside the window.
      h.setNow(DateTime(2026, 8, 24, 23, 0));
      h.setWidgetOpen(false);
      h.service.onHostWidgetClosed();

      expect(h.presenter.shownMessageIds, isEmpty);
      expect(h.service.pendingCampaign, isNull,
          reason: 'dropped, not parked: a quiet window is hours long');
    });
  });
  ...
  group('flushing before an action that leaves the app', () {
    ...
    test('navigate flushes first', () async {
      expect(
          await flushesAfterTapping(const GameballNavigateAction('/cart')), 1,
    ...
    test('a slow flush does not stop the action', () async {
      ...
      expect(h.navigator.pushed, ['/cart'],
          reason:
              'a dead network must not swallow a tap the user is waiting on');
    });
  });
